...
def mulai_input_baru():
    # Menghapus semua input dan variabel terkait
    listnokendaraan.clear()
    listkapasitasmotor.clear()
    listkotatujuan.clear()
    listtariflayanan.clear()
    estimasi_kirim.clear()
    list_tarif_pengiriman.clear()
    listlabel.clear()
    listtotalharga.clear()
    tanggal_pengiriman.clear()
    estimasi_sampai.clear()
    ...
    inputnama['state'] = NORMAL
    inputnik['state'] = NORMAL
    inputjumlah['state'] = NORMAL
    buttonidentitas['state'] = NORMAL

    # Menghapus semua label pada tabel
...
def cetak_nota():
    ...
    pdf.cell(0, 10, 'NOTA PENGIRIMAN', 0, 1, 'C')

    # Menambahkan data pengiriman
    pdf.set_font('Arial', '', 12)
    pdf.cell(0, 10, 'Nama Pengirim: {}'.format(inputnama.get()),0, 1)
    pdf.cell(0, 10, 'NIK Pengirim: {}'.format(inputnik.get()), 0, 1)
    pdf.cell(0, 10, 'Tanggal Pengiriman: {}'.format(inputtanggal.get_date()), 0, 1)
    pdf.cell(0, 10, 'Total Pemesanan: Rp{}'.format(sum(listtotalharga)), 0, 1)
    pdf.cell(0, 10, 'Total Pembayaran: Rp{}'.format(int(inputmembayar.get())), 0, 1)
    pdf.cell(0, 10, 'Kembalian: Rp{}'.format(pembayaran_kembalian()), 0, 1)

    # Menyimpan file PDF
    pdf.output(nama_file)

def selesai():
    result = messagebox.askyesno("Exit", "Apakah anda ingin keluar dari program?")
    if result:
        window_login.destroy()

def logout():
    result = messagebox.askyesno("Log Out", "Apakah Anda yakin ingin log out?")
    if result:
        window_main.withdraw()
        window_login.deiconify()

def on_closing():
    if messagebox.askyesno("Exit", "Apakah Anda ingin keluar dari program?"):
        window_login.destroy()
...
frame_reset = LabelFrame(frame)
frame_reset.grid(row=5, column=0, sticky='news', padx=20, pady=5, columnspan=2)
frame_reset.grid_columnconfigure(1,weight=1)
...
button_frame_bayar = Button(framebayar, text="Bayar", bg='#7ED957', command= lambda : [pembayaran_kembalian(), rincian_nota()])
...
button_cetak = Button(framenota, text='Cetak Nota', command=cetak_nota, bg='#7ED957', padx=10)
button_cetak.grid(row=1, column=2)


buttonmulai = Button(frame_reset, text='Input Baru', command=mulai_input_baru, bg='#7ED957', padx=10)
buttonmulai.grid(row=0, column=0, sticky='news', columnspan=4)

button_selesai = Button(frame_selesai, text="LOGOUT", command= lambda: [logout()], bg="#C70039", padx=10)
...
center_window(window_login)

framelogin = Frame(window_login)
framelogin.pack(padx=10, pady=20)
...
subheading = Label(
    framelogin,
    text='Losik Cargo Program',
    font=('Arial', 16),
    pady=10
)
subheading.grid(row=1, column=0, columnspan=2)

username = Label(framelogin, text='Username', font=('Arial', 12), pady=10)
username.grid(row=2, column=0, sticky="w")

inputusername = Entry(framelogin)
inputusername.grid(row=2, column=1)

password = Label(framelogin, text='Password', font=('Arial', 12), pady=10)
...
buttonlogin.grid(row=4, column=1, sticky='w', pady = 5)       


frame_end = LabelFrame(framelogin)
frame_end.grid(row=6, column=0,columnspan= 2,sticky='news', pady=5)
frame_end.grid_columnconfigure(1,weight=1)
...
button_exit.grid(row=0, column=0, sticky='news', columnspan=4)

window_login.protocol("WM_DELETE_WINDOW", on_closing)
